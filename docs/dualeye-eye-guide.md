...
立创 ESP32-S3 与本眼睛板的联动协议尚未在本分支重新接入；不要沿用旧 ATK 双板 UART 文档中的引脚和供电结论。
...
2. **小智联动：** 旧 ATK 双板 UART 集成已从公共仓库移除；后续如继续使用眼睛板，需要针对立创板重新设计并验证接口。
3. **启动重同步：** ATK 收到 `READY EYE_UART_V1` 后重发当前状态，避免任一主板后启动导致状态丢失。
4. **双轮控制：** 立创板后续计划通过 PCA9685 控制两个 SG5010 连续旋转舵机；先完成独立供电、I²C 扫描和单舵机低速测试。
